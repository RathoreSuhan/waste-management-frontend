 ...
 * friendlier synonyms.
 *
 * Every register, badge and counter renders ReportResponse.status verbatim.
 * The backend now moves a report to IN_PROGRESS the moment its cleanup
 * assignment is claimed, so the lifecycle reads the same for a citizen, a
 * cleaner, an admin and a signed-out visitor. Nothing here infers a status
 * from a role-scoped endpoint, which previously made the same report look
 * PENDING to one reader and IN_PROGRESS to another.
 *
 * Assignment-level detail (PENDING / CLAIMED / IN_PROGRESS / COMPLETED)
 * belongs to the cleaner task screens and is rendered there by
